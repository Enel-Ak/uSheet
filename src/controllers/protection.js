...
function addRangeItem(item) {
  ...
  if (password != null && password.length > 0) {
    passwordTxt =
      '<i class="icon iconfont luckysheet-iconfont-bianji2" title="' +
...
export function initialEvent(file) {
  ...
  //cancel protection
  $(
    "#luckysheet-slider-protection-cancel, #luckysheet-modal-dialog-protection-close"
  ).click(function() {
    closeProtectionModal();
  });
  ...
  $(document)
    .off("click.luckysheetProtection.rangeItemUpdate")
    .on(
      "click.luckysheetProtection.rangeItemUpdate",
      "#luckysheet-protection-rangeItem-container .luckysheet-protection-rangeItem-update",
      function(e) {
        initialProtectionRangeModal();

        isAddRangeItemState = false;
        $("#luckysheet-protection-rangeItem-confirm").html(
          locale_button.update
        );

        openSelfModel("luckysheet-protection-rangeItem-dialog");

        let $rangeItem = $(e.target).closest(
          ".luckysheet-protection-rangeItem"
        );

        let $rangeItemContainer = $(
          "#luckysheet-protection-rangeItem-container"
        );

        let index = $rangeItemContainer
          .find("> div.luckysheet-protection-rangeItem")
          .index($rangeItem);
  ...
  $(document)
    .off("click.luckysheetProtection.rangeItemDelete")
    .on(
      "click.luckysheetProtection.rangeItemDelete",
      "#luckysheet-protection-rangeItem-container .luckysheet-protection-rangeItem-del",
      function(e) {
        let $rangeItem = $(e.target).closest(
          ".luckysheet-protection-rangeItem"
        );
  ...
  $(document)
    .off("click.luckysheetProtection.rangeItemConfirm")
    .on(
      "click.luckysheetProtection.rangeItemConfirm",
      "#luckysheet-protection-rangeItem-confirm",
      function() {
        ...
        if (isAddRangeItemState) {
          ...
          let $range = $rangeitem.find(
            ".luckysheet-protection-rangeItem-range"
          );
  ...
  $(document)
    .off("click.luckysheetProtection.validationConfirm")
    .on(
      "click.luckysheetProtection.validationConfirm",
      "#luckysheet-protection-sheet-validation-confirm",
      function(e) {
  ...
  $("#luckysheet-protection-check-selectLockedCells").change(function() {
    let $selectLockedCells = $(
        "#luckysheet-protection-check-selectLockedCells"
      ),
      $selectunLockedCells = $(
        "#luckysheet-protection-check-selectunLockedCells"
      );
  ...
  $(document)
    .off("click.luckysheetProtection.dvRange")
    .on(
      "click.luckysheetProtection.dvRange",
  ...
  $(document)
    .off("click.luckysheetProtection.dvRange2")
    .on(
      "click.luckysheetProtection.dvRange2",
      "#luckysheet-protection-rangeItem-dialog .show-box-item-dropdown .range .fa-table",
      function(e) {
        $("#luckysheet-protection-rangeItem-dialog").hide();
  ...
  $(document)
    .off("click.luckysheetProtection.dvRangeConfirm")
    .on(
      "click.luckysheetProtection.dvRangeConfirm",
      "#luckysheet-dataVerificationRange-dialog-confirm",
      function(e) {
        let txt = $(this)
          .parents("#luckysheet-dataVerificationRange-dialog")
          .find("input")
          .val();
  ...
  $(document)
    .off("click.luckysheetProtection.dvRangeClose")
    .on(
      "click.dvRangeClose",
  ...
  $(document).on(
    "click.luckysheetProtection.luckysheetProtection",
...
export function openProtectionModal(file) {
  ...
  if (file != null && file.config != null && file.config.authority != null) {
    ...
    //protection initial config //没有设置可编辑区域
    //默认全选
    ...
    $("#luckysheet-protection-check-usePivotTablereports").prop(
      "checked",
      true
    );
...
function openRangePasswordModal(rangeAut) {
    ...
    $("body").append(
      replaceHtml(modelHTML, {
        ...
        content: `
            ...
            `,
        botton: `<button id="luckysheet-protection-range-validation-confirm" class="btn btn-primary el-button el-button--primary">${locale_button.confirm}</button>
                        <button class="btn btn-default luckysheet-model-close-btn el-button">${locale_button.cancel}</button>`,
        style: "z-index:100003",
      })
    );
  }
  ...
  $("#luckysheet-protection-range-validation-confirm")
    .off("click")
    .on("click", function() {
      let password = $input.val();
